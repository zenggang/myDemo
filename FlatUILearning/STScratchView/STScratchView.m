...
{
    centerPoint=CGPointMake(hideView.frame.size.width/2, hideView.frame.size.height/2);
    isShowOut=NO;
...
- (void)scratchTheViewFrom:(CGPoint)startPoint to:(CGPoint)endPoint
{
    float scale = [UIScreen mainScreen].scale;
    if (!isShowOut) {
        float pointToMainDistance=distanceBetweenPoints(endPoint, centerPoint);
        if (pointToMainDistance<30) {
            to_mainArea_time++;
            if (to_mainArea_time>5) {
                isShowOut=YES;
                [_delegate didShowTheMainArea];
            }
        }
    }
